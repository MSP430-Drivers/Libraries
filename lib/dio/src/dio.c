...
t_DioInst DIO_v_SetInstance(t_Port e_port, t_Pin e_pin, t_PinDir e_dir)
{
  ...
  GPIO_v_SetUpPin(e_port, e_pin, e_dir, e_PIN_DIO);
...
{
  if(e_INPUT == t_dio.e_dir)
  {
    REG_v_SetBitIn8BitReg(pu_PXIE_ADDR[t_dio.e_port], t_dio.e_pin);
    (e_RISING_EDGE == e_edgeMode) ? REG_v_ClearBitIn8BitReg(pu_PXIES_ADDR[t_dio.e_port], t_dio.e_pin) : REG_v_SetBitIn8BitReg(pu_PXIES_ADDR[t_dio.e_port], t_dio.e_pin);
  }
  else
  {
...
void DIO_v_PinIntDis(t_DioInst t_dio)
{
  if(e_INPUT == t_dio.e_dir)
  {
    if(e_RES_EN == REG_u_GetBitIn8BitReg(pu_PXIE_ADDR[t_dio.e_port], t_dio.e_pin))
    {
