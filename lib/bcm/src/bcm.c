...
{
  REG_SetReg8Bits(u_DCOCTL_ADDR, REG_u_GetReg8Bits(pu_CALDCO_XMHZ_ADDR[e_freq]));  // DCOCTL -> DCO and MOD
  REG_SetReg8Bits(u_BCSCTL1_ADDR, REG_u_GetReg8Bits(pu_CALBC1_XMHZ_ADDR[e_freq])); // BCSCTL1 -> RSEL
}
...
{
  REG_v_SetBitsReg8Bits(u_BCSCTL2_ADDR, 7u, 6u, e_srcClk);
  REG_v_SetBitsReg8Bits(u_BCSCTL2_ADDR, 5u, 4u, e_div);
}
...
{
  REG_v_SetBitsReg8Bits(u_BCSCTL2_ADDR, 3u, 3u, e_srcClk);
  REG_v_SetBitsReg8Bits(u_BCSCTL2_ADDR, 2u, 1u, e_div);
}
...
void BCM_v_ACLKConf(t_Divider e_div)
{
  ...
  REG_v_SetBitsReg8Bits(u_BCSCTL1_ADDR, 5u, 4u, e_div);
}
